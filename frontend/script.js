...
const ATTACK_CHAIN = [
    { id:0, name:"Reconnaissance", fullName:"Network Reconnaissance", risk:22, prob:0, conf:0, next:null, mitreTactic:"Reconnaissance", description:"Passive network enumeration detected", events:["🔍 Port scanning detected","📡 DNS probing activity"], alert:"Monitoring reconnaissance phase", anomaly:0.15 },
    { id:1, name:"Network Scan", fullName:"Active Scanning & Probing", risk:38, prob:0.45, conf:0, next:"Initial Access", mitreTactic:"Reconnaissance", description:"Intensified network scanning detected", events:["🎯 Multiple port connections","⚠️ Service fingerprinting"], alert:"Escalating reconnaissance activity", anomaly:0.38 },
    { id:2, name:"Initial Access", fullName:"Exploitation & Access Gained", risk:55, prob:0.60, conf:0.72, next:"Lateral Movement", mitreTactic:"Initial Access", description:"System compromise confirmed", events:["🔓 Unauthorized access","💥 Payload execution"], alert:"CRITICAL: Initial access compromised", anomaly:0.55 },
    { id:3, name:"Lateral Movement", fullName:"Internal System Propagation", risk:68, prob:0.72, conf:0.81, next:"Command and Control", mitreTactic:"Lateral Movement", description:"Lateral movement in progress", events:["🔀 Internal lateral connections","👤 Privilege escalation attempt"], alert:"ALERT: Attacker spreading through network", anomaly:0.68 },
    { id:4, name:"Command & Control", fullName:"Malicious C2 Communication", risk:78, prob:0.78, conf:0.89, next:"Data Exfiltration", mitreTactic:"Command and Control", description:"C2 communication established", events:["📤 Suspicious outbound traffic","🎯 C2 beacon detected"], alert:"CRITICAL: C2 communication confirmed", anomaly:0.78 },
    { id:5, name:"Data Exfiltration", fullName:"Sensitive Data Extraction", risk:74, prob:0.61, conf:0.75, next:null, mitreTactic:"Exfiltration", description:"Data breach in progress", events:["🗂️ Database query surge","📦 Large data transfer"], alert:"CRITICAL: Data exfiltration underway", anomaly:0.74 }
];
...
function initCharts(){
    ...
    CHARTS.traffic = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                { label: 'Incoming Traffic', data: STATE.trafficHistory.map(d=>d.incoming), borderColor: '#818cf8', backgroundColor: 'rgba(129, 140, 248, 0.10)', tension: 0.4, fill: true, pointRadius: 0, borderWidth: 2.5 },
                { label: 'Outgoing Traffic', data: STATE.trafficHistory.map(d=>d.outgoing), borderColor: '#a855f7', backgroundColor: 'rgba(168, 85, 247, 0.10)', tension: 0.4, fill: true, pointRadius: 0, borderWidth: 2.5 }
            ]
        },
        options: {
            responsive: true,
            ...
            scales: {
                y: { beginAtZero: false, grid: { color: 'rgba(168,85,247,0.08)' }, ticks: { color: '#948ba8', font: { family:'Rajdhani' } } },
...
function applyBackendUpdate(data){
    if(!data || data.type !== 'network_update') return;

    const previousRisk = STATE.risk;

    STATE.risk = Number(data.risk ?? STATE.risk);
    STATE.flows = Number(data.flows ?? STATE.flows);
    STATE.packets = Number(data.packets ?? STATE.packets);
    STATE.anomaly = Number(data.anomaly ?? STATE.anomaly);
    STATE.trafficHistory.push({
        incoming: Math.max(200, Number(data.packets ?? STATE.packets)),
        outgoing: Math.max(150, Number(data.flows ?? STATE.flows) * 20)
    });

    if(STATE.trafficHistory.length > 24) STATE.trafficHistory.shift();

    const stageIndex = ATTACK_CHAIN.findIndex(stage =>
        stage.next === data.next_attack ||
        stage.name.toLowerCase() === String(data.next_attack || '').toLowerCase()
    );

    if(stageIndex >= 0){
        STATE.step = stageIndex;
    }

    updateUI();

    const color = STATE.risk < 40 ? "#34d399" : STATE.risk <= 70 ? "#fbbf24" : "#fb7185";

    if(previousRisk !== STATE.risk || data.event){
        showToast(
            data.event || "Network update",
            `Risk ${STATE.risk}% · Next ${data.next_attack || "—"} · MITRE ${data.mitre || "—"}`,
            color
        );
    }

    const lastUpdate = document.getElementById("lastUpdate");
    if(lastUpdate) lastUpdate.textContent = data.time || formatTime();
}
...
function setConnectionStatus(connected){
    ...
    const card = document.getElementById("statusCardGlow");

    if(!status || !indicator || !desc || !card) return;
    ...
    if(connected){
        ...
        desc.textContent = "Backend WebSocket connected";
        card.classList.remove("glow-amber","glow-red");
        card.classList.add("glow-green");
    } else {
        indicator.style.background = "#fbbf24";
        indicator.style.boxShadow = "0 0 10px #fbbf24";
        status.style.color = "#fbbf24";
        status.textContent = "OFFLINE";
        desc.textContent = "Waiting for backend connection";
...
function connectWebSocket(){
    ...
    const wsUrl = `${protocol}//${host}${backendPort}/ws/alerts`;
...
function updateUI(){
    ...
    if(statusLabelEl.textContent !== status && statusLabelEl.textContent !== "LIVE"){
    ...
    const threatLabel = STATE.risk < 40 ? "LOW" : STATE.risk <= 70 ? "MEDIUM" : "CRITICAL";
    ...
    for(let i = 0; i < 5; i++){
        const bar = document.getElementById(`bar${i}`);
        if(bar) bar.style.height = Math.max(15, Math.min(95, 45 + STATE.risk*0.5 + rand(-18,18))) + "%";
    }
}
